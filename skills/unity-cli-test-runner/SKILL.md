...
| 2 | 起動中エディタが見つからない、リトライ予算（15秒）を使い切ってもなお `unity cmd editor_status` が失敗/タイムアウト | Unityエディタが起動していないか未接続。ユーザーに手動起動を依頼する（ドメインリロード中の一時的な切断はスクリプト内部で自動リトライ済みなので、ここに到達した場合は本当に未起動/未接続の可能性が高い）。**ただし標準エラー出力に `Server Busy` / `blocked_by_dialog` が含まれる場合は未起動ではない** — エディタはモーダルダイアログでブロックされているので、起動ではなく**ダイアログを閉じてもらう**よう依頼する（pipeline 0.6以降。「既知の罠」参照） |
