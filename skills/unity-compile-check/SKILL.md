...
| 2 | 起動中エディタが見つからない、リトライ予算を使い切っても失敗/タイムアウト | Unityエディタが起動していないか未接続。呼び出し元（人間またはコントローラー）に手動起動・接続確認を依頼する。**ただし標準エラー出力に `Server Busy` / `blocked_by_dialog` が含まれる場合は未起動ではない** — エディタがモーダルダイアログでブロックされているので、ダイアログを閉じてもらうよう依頼する（`com.unity.pipeline` 0.6以降） |
